...
| `assets/characters/Base_Male.*`, `Superhero_Female_FullBody.*`, `Hair_SimpleParted.*`, `Hair_Buns.*`, `T_Superhero_*`, `T_Hair_*`, `T_Eye_*`, baked into `apps/web/public/models/wardrobe.glb` | [Universal Base Characters](https://quaternius.com/packs/universalbasecharacters.html) by Quaternius (Standard) | CC0 1.0 Universal | The player character: two base bodies, each with its own 65-joint skeleton, eyes, brows and one hairstyle. The pack itself is not served - `tools/build_wardrobe.py` repacks it into one glb |
...
- `Superhero_Male_FullBody.gltf` is checked in as `Base_Male.gltf`, buffer uri renamed to match. Each body keeps the atlas its own pack ships - the male `T_Superhero_Male_*`, the female `T_Superhero_Female_*`. **Do not substitute another pack's male atlas.** The outfit pack's `T_Regular_Male_*` was once wired here on the assumption that both males share one unwrap; they do not, and the mismatch was invisible in nine tenths of the body while painting the underwear across an inner thigh, a foot and the trapezius.
- `Superhero_Female_FullBody.gltf` shipped pointing at `T_Eye_Normal_png.png`, a filename the pack does not contain. It is repointed at `T_Eye_Normal.png`.
