...
- **Sound is a snapshot DIFF** (`audio/soundscape.ts`), never a call at the dispatch site: a cue fired on the press lies whenever the sim refuses the cast. One bus (`audio/bus.ts`), or the volume slider covers half the game. Masters are CURATED, never generated: every cue names a Sonniss library file in `SOURCES` (`tools/import_sfx.py`), which resamples it and runs `tools/trim_sfx.py` into `public/audio/`. Generation is what made a stone construct and a bog thing die the same way. Never feed Sonniss audio to a model — the licence forbids training or conditioning on it; shipping it in the game is fine, no attribution required.
...
- **`docs/todo/TODO.md` is his, edited continuously** (gitignored, so re-read it each session). New items land under `## SORT`: rate each and move it into the High / Mid / Low sections, ranked inside the tier so the FIRST line is the most important and done items sink to the bottom. Tick `[x]` when it ships. Never reword or drop his items.
