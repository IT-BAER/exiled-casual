...
describe("registerDeath", () => {
  ...
  it("levels a free attack on a bar that holds nothing else", () => {
    ...
    const { sim, world, sessionE } = makeGemKill({
      area: "map", areaTier: 1, level: 10, xp: 0,
      skills: {
        gems: { "skill.snap_shot.v1": { level: 1, xp: 0 } },
        bar: [null, null, null, null, null, MOVE_SOCKET, null, "skill.snap_shot.v1"],
      },
    });
    sim.step([]);
  ...
  it("pays nothing to a free attack the character owns but has not slotted", () => {
    // Slotted, not cast, and not merely owned: the rule that governs every
    // other gem governs this one too.
    const { sim, world, sessionE } = makeGemKill({
      area: "map", areaTier: 1, level: 10, xp: 0,
      skills: {
        gems: {
    ...
    sim.step([]);
    expect(world.get<SkillsC>(sessionE, "skills")!.gems["skill.snap_shot.v1"]).toEqual({ level: 1, xp: 0 });
  });
  ...
  describe("scroll drops", () => {
    ...
    function makeMapKills(mapSeed: number) {
      ...
      const w = sim.world;
      const s = w.create();
      w.set(s, "session", { area: "map", atlasSeed: 1, mapSeed, waystoneSeed: 0, areaTier: 5, activeNodeId: "n", completedNodes: [], portalsLeft: 6, mapOpen: 1, pendingArea: "" });
      for (let i = 0; i < 7; i++) {
        const m = w.create();
        w.set(m, "position", { x: fp(i), y: fp(0) });
        w.set(m, "health", { life: 0, maxLife: 40 });
...
describe("clearing a map hands Waystones back", () => {
  ...
  function makeBossDeath(area: "map", activeNodeId: string, completedNodes: string[], areaTier = 5) {
    ...
    world.set(boss, "health", { life: 0, maxLife: fp(500) });
    world.set(boss, "boss", { phase: 1, nextAbilityTick: 0, spawnX: 0, spawnY: 0, rootedUntilTick: 0 });
    // Position required: death.ts only spawns drops when bossPos is set.
    world.set<Position>(boss, "position", { x: fp(0), y: fp(0) });
    return { sim, world, sessionE };
  }

  const waystoneGroundItems = (world: ReturnType<typeof makeBossDeath>["world"]) =>
    world.query("item", "position").filter((e) =>
      (world.get(e, "item") as { item: { waystone?: unknown } }).item.waystone !== undefined,
    );
  ...
  it("the boss's death adds the run's drops to the owned stock", () => {
    ...
    for (const e of drops) {
      const tier = (world.get(e, "item") as { item: { waystone: { tier: number } } }).item.waystone.tier;
